...
public:
    ...
    std::vector<std::tuple<int, float, Rect>> predict(vector<cv::Mat> &imgs)
    {
        ...
        for (int i = 0; i < batch_size_; ++i)
        {
            ...
            postprocess(sliced_output, imgs[i], i);
            count_++;
        }
        cudaFree(d_input);
        cudaFree(d_output);
...
private:
    ...
    void postprocess(const std::vector<float> &output, Mat &img, int mountIdx)
    {
        ...
        std::vector<Rect> boxes_tire;   // 타이어용 바깥 바운딩 박스를 저장할 벡터
        ...
        for (int idx : nmsIndices)
        {
            ...
            // if(boxes[idx].width/boxes[idx].height < aspectRatio) {
            //     continue;
            // }
            ...
            // 평가용
            float tire_margin_ratio=0.25; //
            boxes[idx].x -= tire_margin_ratio*boxes[idx].width/2.0;
            boxes[idx].width = (1 + tire_margin_ratio)*boxes[idx].width;
            boxes[idx].y -= tire_margin_ratio*boxes[idx].height/2.0;
            boxes[idx].height = (1 + tire_margin_ratio)*boxes[idx].height;
            rectangle(img, boxes[idx], Scalar(255, 255, 255), 2); // 바운딩 박스를 그린다
            cv::Mat imgPointMat = (cv::Mat_<double>(3, 1) << boxes[idx].x + boxes[idx].width / 2,
                                   boxes[idx].y + boxes[idx].height,
                                   1.0);
            cv::Mat homography = cv::Mat(3, 3, CV_64F);
            if(mountIdx == 0) { // front
                homography = (cv::Mat_<double>(3, 3) << -0.001440296, -0.002125581, 2.539575267,
                              -0.000040410, -0.000143478, -1.407735065,
                              -0.000010410, -0.001817845, 1.0);
            }
            else if(mountIdx == 1) { // rear
                homography = (cv::Mat_<double>(3, 3) << -0.001564882829719506, -0.008843939711523505, 5.761559384110413,
                              3.663173893837778e-06, -0.0004775357468988286, -1.463342883099236,
                              -1.814755898874596e-06, -0.00204689188608664, 1.0);
            }
            cv::Mat groundPointMat = homography*imgPointMat;
            groundPointMat /= groundPointMat.at<double>(2, 0);
            circle(img, cv::Point2f(imgPointMat.at<double>(0, 0), imgPointMat.at<double>(1, 0)), 5, cv::Scalar(0, 0, 255), -1);
            std::string text = "POINT=(" + std::to_string(groundPointMat.at<double>(0, 0))+", "+std::to_string(groundPointMat.at<double>(1, 0))+")" ;
            putText(img, text, Point(boxes[idx].x, boxes[idx].y+boxes[idx].height+10), FONT_HERSHEY_SIMPLEX, 1.0, Scalar(0, 255, 0), 3); // 텍스트 추가
            text = "DISTANCE=" + std::to_string(std::sqrt(std::pow(groundPointMat.at<double>(0, 0), 2) + std::pow(groundPointMat.at<double>(1, 0), 2)));
            putText(img, text, Point(boxes[idx].x, boxes[idx].y+boxes[idx].height+50), FONT_HERSHEY_SIMPLEX, 1.0, Scalar(0, 255, 255), 3); // 텍스트 추가
        }
    }
};
